...
const URI = 'https://utpitosbackend.onrender.com/products/';//esta sera la ruta a la cual se generaran peticiones en este caso sera para los productos
...
export const ShopContextProvider = (props) => {
    ...
    const addToCart = async (itemId) => { //funcion para poder agregar al carrito enviando como parametro el id del producto y poder reservarlo en el servidor
        await axios.get('https://utpitosbackend.onrender.com/products/book/'+ itemId + '?f=book')//se genera una peticion get para poder traer el el producto el cual se va reservar el producto
        .then(({ data }) => {
    ...
    const removeFromCart = async (itemId) => { //funcion para remover del carrito a partir del id
        await axios.get('https://utpitosbackend.onrender.com/products/book/'+ itemId + '?f=unbook')//en esta ruta se hace la peticion
        .then(({ data }) => {
